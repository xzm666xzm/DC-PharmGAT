"""
ESR1 ChEMBL External Validation Dataset Builder
=================================================
text ChEMBL details ESR1 (CHEMBL206) of information test active data,
...
details need point:
  - target details: CHEMBL206 (Estrogen receptor / ER-alpha / ESR1, SINGLE PROTEIN, Homo sapiens)
  - active type: IC50 / Ki / Kd, assay_type='B', confidence>=8
  - information reporter/transcription/agonist/antagonist text success text property assay
  - text first save to strict_binding data, details generate broad_B_assay data as details
  ...
  - negative summary ESR1 assay text test text active molecules
=================================================
"""
...
class ESR1ChEMBLCurator:
    """ESR1 ChEMBL details validation dataset build text"""
    ...
    STRICT_KEYWORDS = ['binding', 'binds', 'radioligand', 'competition', 'competitive', 'affinity', 'spa', 'displacement']
    EXCLUDE_KEYWORDS = ['reporter', 'luciferase', 'transcription', 'ere', 'activation', 'agonist', 'antagonist', 'cell proliferation', 'viability']
    ...
    RAW_FIELDS = [
        ...
        "data_validity_comment", "assay_description"
]

    def __init__(self, output_dir: str = "outputs/esr1_chembl_external"):
        ...
        self._log_header("ESR1 ChEMBL External Validation Builder")
        self._log(f"target: {self.TARGET_CHEMBL_ID} (ESR1)")
    ...
    def collect_raw_activities(self) -> pd.DataFrame:
        """text ChEMBL get ESR1 (CHEMBL206) of Binding assay active details. 
        information point details: information directory already exists have effect of raw CSV, details read details API get. 
        """
        self._log_step(1, "text ChEMBL details ESR1 original active data")

        raw_csv = os.path.join(self.output_dir, "esr1_chembl_raw_activities.csv")
        ...
        stats = {"no_type": 0, "no_unit": 0, "bad_value": 0,
                 "no_id_smi": 0, "invalid_data": 0, "excluded_assay": 0}
        ...
        for r in raw:
            ...
            # assay_description filtering
            desc = str(r.get("assay_description", "")).lower()
            if any(k in desc for k in self.EXCLUDE_KEYWORDS):
                stats["excluded_assay"] += 1
                continue
                
            r["is_strict"] = any(k in desc for k in self.STRICT_KEYWORDS)
            kept.append(r)
        ...
        self._log(f"    assay_description exclude: {stats['excluded_assay']}")
    ...
    def curate_records(self, df_raw: pd.DataFrame, mol_meta: dict, prefix: str) -> pd.DataFrame:
        ...
        for cid, rows in groups.items():
            ...
            st_types = sorted(set(str(r["standard_type"]) for r in rows if r.get("standard_type")))
            assay_ids = sorted(set(str(r["assay_chembl_id"]) for r in rows if r.get("assay_chembl_id")))
            doc_ids = sorted(set(str(r["document_chembl_id"]) for r in rows if r.get("document_chembl_id")))
            assay_descs = sorted(set(str(r["assay_description"]) for r in rows if r.get("assay_description")))
            ...
            records.append({
                ...
                "assay_descriptions": " | ".join(assay_descs),
        ...
        out_path = os.path.join(self.output_dir, f"{prefix}_curated_molecule_level.csv")
        df.to_csv(out_path, index=False)
        self._log(f"  saved: {out_path}")
        return df

    # ─── Step 4: details ─────────────────────────────────────────────────
    def export_outputs(self, df: pd.DataFrame, prefix: str):
        ...
        p = os.path.join(self.output_dir, f"{prefix}_external_active.ism")
        ...
        p = os.path.join(self.output_dir, f"{prefix}_external_inactive.ism")
        with open(p, "w", encoding="utf-8") as f:
        ...
        p = os.path.join(self.output_dir, f"{prefix}_conflicts.csv")
        df_conf.to_csv(p, index=False)
        self._log(f"  conflict -> {p}  ({len(df_conf)} molecules)")

        # ambiguous
        df_amb = df[df["activity_label"] == "ambiguous"].copy()
        p = os.path.join(self.output_dir, f"{prefix}_ambiguous_midrange.csv")
        ...
        p = os.path.join(self.output_dir, f"{prefix}_label_summary.json")
        with open(p, "w", encoding="utf-8") as f:
            json.dump(summary, f, indent=2, ensure_ascii=False)
        self._log(f"  summary  -> {p}")

        # summary
        self._log_header(f"Final Summary: {prefix}")
        ...
        self._log("All inactive molecules are from ESR1 assay measured data.")

    # ─── information ────────────────────────────────────────────────────────
    def run(self):
        """run summary"""
        self._log_header("Start building ESR1 ChEMBL External Validation")
        t0 = time.time()
        ...
        # split other process Strict Binding and Broad B Assay
        df_raw_strict = df_raw[df_raw["is_strict"] == True].copy()
        
        self._log_step(3, "molecule information & information note (Strict Binding)")
        df_curated_strict = self.curate_records(df_raw_strict, mol_meta, prefix="esr1_chembl")
        if not df_curated_strict.empty:
            self._log_step(4, "summary file (Strict Binding)")
            self.export_outputs(df_curated_strict, prefix="esr1_chembl")
            
        self._log_step(3, "molecule information & information note (Broad B Assay)")
        df_curated_broad = self.curate_records(df_raw, mol_meta, prefix="esr1_chembl_broad")
        if not df_curated_broad.empty:
            self._log_step(4, "summary file (Broad B Assay)")
            self.export_outputs(df_curated_broad, prefix="esr1_chembl_broad")

        elapsed = time.time() - t0
        self._log(f"\nTotal time: {elapsed:.1f}s")
        self._log("Done!")
        return df_curated_strict


def main():
    curator = ESR1ChEMBLCurator(output_dir="outputs/esr1_chembl_external")
    curator.run()
